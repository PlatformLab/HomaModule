	...
	__must_hold(rpc->bucket->lock)
{
	...
	while (bytes_left > 0 && shinfo->nr_frags < MAX_SKB_FRAGS &&
		...
		frag_bytes = umin(skb_frag_size(msg_frag) - bytes_to_skip,
				 bytes_left);
		page = skb_frag_page(msg_frag);
		get_page(page);
		skb_frag->page.p = page;
		skb_frag->page_offset = msg_frag->page_offset + bytes_to_skip;
