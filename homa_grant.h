 ...
 */
static inline int homa_grant_lock(struct homa *homa, int recalc)
	__acquires(&homa->grant_lock)
{
	...
	if (spin_trylock_bh(&homa->grant_lock))
		...
		result = homa_grant_lock_slow(homa, recalc);
	homa->grant_lock_time = sched_clock();
	return result;
}
 ...
 * @homa:    Overall data about the Homa protocol implementation.
 */
static inline void homa_grant_unlock(struct homa *homa)
	__releases(&homa->grant_lock)
{
	INC_METRIC(grant_lock_ns, sched_clock() - homa->grant_lock_time);
	spin_unlock_bh(&homa->grant_lock);
}
