...
# nic_data_pkts:     List of packets handed off to the NIC for this RPC, sorted
#                    in order of 'nic'
...
# records:      Number of trace records in the file
traces = {}

# IP address -> node name. Computed by AnalyzeRpcs and AnalyzePackets.
# A single node may be in the table twice, once using the full 4-byte
# IP address, such as "0xa000105", which is available for Homa packets,
# and once using only the low-order two bytes (e.g. "0x0105") which is
# all that is available in TCP packets.
ip_to_node = {}
...
#               IPv4 address
# dest:         Destination port for the packet; same format as source.
# length:       # bytes of message data in the received packet
# seq_ack:      If length is non-zero then this is the sequence number of
#               the first byte of data in the packet; otherwise it is the
#               acknowledgment sequence number.
# tso_length:   The number of data bytes in the packet (before TSO). This
#               field is not present in packets that are generated by TSO.
# segments:     This field will be present in the first packet of each
#               TSO packet (the one with tso_length set); it will be a
#               list of all the other segments deriving from the same
#               TSO packet.
...
# xmit2:        qdisc_xmit if it exists, otherwise xmit: a time when Homa
#               has decided to transmit the packet (after any Homa queuing)
# tx_core:      Core on which ip*xmit was invoked for the packet
# qdisc_xmit:   Time when homa_qdisc requeued a packet that was deferred
#               because of NIC queue length (only present for deferred
#               packets)
# nic:          Time when the the packet was handed off to the NIC
...
# This variable holds information about every identifiable RPC sent via
# TCP in the traces. It is created by the tcp_rpcs analyzer. Keys are
# unique identifiers for RPCs in the form 'client server req_seq' with
# the meanings described below. Each value is a dictionary with the following
# entries:
# client:        Client port: hex string whose low 16 bits are the port number,
...
#                the request
# req_seq:       Sequence number of the first byte of the request message
# req_length:    Size of the request message in bytes
# req_end_seq:   req_seq + req_length
# req_pkts:      List of request packets, in the order sent. Includes only
#                data packets
# req_recvd:     Time when tcp_recvmsg returned on the server
# resp_send:     Time when tcp_sendmsg was invoked for the first bytes of
#                the response
# resp_seq:      Sequence number of the first byte of the response message
# resp_length:   Size of the response message in bytes
# resp_end_seq:  resp_seq + resp_length
# resp_pkts:     List of response packets, in the order sent. Includes only
#                data packets
# resp_recvd:    Time when tcp_recvmsg returned on the client (RPC complete)
tcp_rpcs = {}
...
# Total header bytes in a TCP packet (TCP and IP headers, assuming IPv4).
tcp_hdr_length = ipv4_hdr_length + 20
...
def get_range(s, option_name=None, parse_float=False, one_value=True):
    ...
    s:            The input string to parse; may contain either one or
                  two values
    option_name:  If specified, contains the name of the option that was
                  specified as range; used for error messages
    parse_float:  True means parse values as floating-point numbers; False
                  means integers
    one_value:    True means it is OK for s to contain only one value, in
                  which case it is returned as the lower end of the range,
                  with None as the upper end.
    """
    ...
    if len(values) == 1:
        if not one_value:
            if option_name != None:
                raise Exception('Bad %s spec \'%s\'; must contain two values' %
                        (option_name, s))
            else:
                raise Exception('Bad range spec \'%s\'; must contain two values'
                        % (s))
        min = float(values[0]) if parse_float else int(values[0])
        return [min, None]
    if len(values) == 2:
        min = float(values[0]) if parse_float else int(values[0])
        max = float(values[1]) if parse_float else int(values[1])
        return [min, max]
    if option_name != None:
        raise Exception('Bad %s spec \'%s\'; must be \'value\' or '
                '\'value1 value2\'' % (option_name, s))
    else:
...
def print_pkts(pkts, header=True, comment=False):
    ...
    if header:
        ...
        buf.write(prefix + 'Id/Seq:     RPC identifier for Homa packet on '
                'sender, sequence\n')
    ...
    for pkt in pkts:
        xmit = pkt['xmit'] if 'xmit' in pkt else None
        if 'qdisc_xmit' in pkt:
        ...
        if pkt['type'] == 'data' or pkt['type'] == 'grant':
            if pkt['id'] in rpcs:
                ...
                if 'out_length' in rpc:
                    msg_length = '%d' % (rpc['out_length'])
            id_string = '%d' % (pkt['id'])
            offset_string = '%d' % (pkt['offset'])
        else:
        ...
        line += ' %10s %10s  %3s %10s %7s' % (print_if(xmit, '%.3f'),
                ...
                nic_delay_string)
        line += '  %10s %7s %10s %7s %4s %2s' % (print_if(gro, '%.3f'),
                gro_delay_string, print_if(free, '%.3f'),
                free_delay_string, prio_string, rx_msg)
...
def print_tcp_rpcs(rpcs, header=True):
    ...
    rpcs:        RPCs to print; must be entries in tcp_rpcs
    header:      If True then the result will include initial text describing
                 the fields printed on each line.
    """
    buf = StringIO()
    if header:
        buf.write('Start:      Time when tcp_sendmsg was invoked for request\n')
        buf.write('Client:     Node that sent the RPC request\n')
        buf.write('Server:     Node that handled the RPC and sent response\n')
        buf.write('Length:     Length of request message\n')
...
def set_tcp_ip_node(tcp_endpoint, node):
    ...
    tcp_endpoint:  An endpoint spec from a TCP packet. Must be a hex string
                   (with leading "0x") whose high-order 16 bits are the
                   low-order 16 bits of an IPv4 address.
    node:          Name of the node corresponding to tcp_endpoing
    """
    key = tcp_endpoint[:-4]
    ip_to_node[key] = node
...
class Dispatcher:
    ...
    def __init__(self):
        ...
        # trace entry is seen.
        self.core_saved = defaultdict(dict)
    ...
    def __gro_ctl(self, trace, time, core, match, interests):
        ...
        type = match.group(3)
        for interest in interests:
            interest.tt_gro_ctl(trace, time, core, peer, id, type)
    ...
    def __free_grant(self, trace, time, core, match, interests):
        ...
        qid = int(match.group(3))
        for interest in interests:
            interest.tt_free_grant(trace, time, core, id, offset, qid)

    patterns.append({
        'name': 'free_grant',
        'regexp': 'freeing tx skb for homa grant, id ([0-9]+), '
                'offset ([0-9]+), qid ([0-9]+)'
    })
    ...
    def __tcp_sendmsg(self, trace, time, core, match, interests):
        ...
        for interest in interests:
            interest.tt_tcp_sendmsg(trace, time, core, source, dest, msg_length,
                    sequence, saved['sendmsg_slot'], saved['sendmsg_response'])

    patterns.append({
        'name': 'tcp_sendmsg',
        'regexp': 'tcp_sendmsg invoked for message from (0x[a-f0-9]+) to '
    ...
    def __tcp_xmit(self, trace, time, core, match, interests):
        ...
        for interest in interests:
            interest.tt_tcp_xmit(trace, time, core, source, dest, data_bytes,
                    seq_ack)

    patterns.append({
        'name': 'tcp_xmit',
        'regexp': 'Transmitting TCP packet from (0x[a-f0-9]+) to '
    ...
    def __tcp_qdisc(self, trace, time, core, match, interests):
        ...
        seq_ack = int(match.group(4))
        for interest in interests:
            interest.tt_tcp_qdisc(trace, time, core, source, dest, data_bytes,
                    seq_ack)

    patterns.append({
        'name': 'tcp_qdisc',
        'regexp': 'homa_qdisc_pacer requeued TCP packet from (0x[a-f0-9]+) to '
                '(0x[a-f0-9]+), data bytes ([0-9]+), seq/ack ([0-9]+)'
    })
    ...
    def __tcp_nic(self, trace, time, core, match, interests):
        ...
        for interest in interests:
            interest.tt_tcp_nic(trace, time, core, source, dest, data_bytes,
                    seq_ack)
    ...
    def __tcp_gro(self, trace, time, core, match, interests):
        ...
        seq_ack = int(match.group(4))
        for interest in interests:
            interest.tt_tcp_gro(trace, time, core, source, dest, data_bytes,
                    seq_ack)
    patterns.append({
        'name': 'tcp_gro',
        'regexp': 'tcp_gro_receive got packet from (0x[a-f0-9]+) to '
                '(0x[a-f0-9]+), data bytes ([0-9]+), seq/ack ([0-9]+)'
    })
    ...
    def __tcp_recvmsg(self, trace, time, core, match, interests):
        ...
        sequence = int(match.group(4))
        for interest in interests:
            interest.tt_tcp_recvmsg(trace, time, core, source, dest, msg_length,
                    sequence)

    patterns.append({
        'name': 'tcp_recvmsg',
        'regexp': 'tcp_recvmsg returning message from (0x[a-f0-9]+) to '
    ...
    def __freeze_rx(self, trace, time, core, match, interests):
        ...
        for interest in interests:
            interest.tt_freeze_rx(trace, time, core, saddr, sport)
...
class AnalyzeDelay:
    ...
    def print_pkt_delays(self):
        ...
        for p, pkt in packets.items():
            if (pkt['msg_length'] != None) and (pkt['msg_length'] <= short_limit):
        ...
        print('-------------------------------------------------------------------------')
        print('Data packets from messages <= %d bytes:' % (short_limit))
        ...
        print('\nData packets from messages > %d bytes:' % (short_limit))
...
class AnalyzeFilter:
    ...
    def filter_packets(self, options):
        ...
        if options.filter != None:
            ...
            filter_func = getattr(self, name, None)
        ...
        if options.grolat != None:
            min_gro, max_gro = get_range(options.grolat,
                    option_name='--grolat', parse_float=True, one_value=True)
            if max_gro == None:
                max_gro = 1e20
    ...
    def output(self):
        ...
        for pkt in pkts:
            ...
            if 'softirq' in pkt and 'xmit' in pkt:
                total.append(pkt['softirq'] - pkt['xmit'])
            elif 'gro' in pkt and 'xmit' in pkt:
                total.append(pkt['gro'] - pkt['xmit'])
            if 'nic' in pkt and 'free_tx_skb' in pkt:
        ...
        def print_pcts(data, label):
            ...
            if not data:
                ...
                print('%-10s %6d %6.1f %6.1f %6.1f %6.1f %6.1f %6.1f %6.1f' % (label,
                    len(data), data[0], data[10*len(data)//100],
                    data[50*len(data)//100], data[90*len(data)//100],
                    data[99*len(data)//100], data[len(data)-1],
                    sum(data)/len(data)))

        print('Phase       Count    Min    P10    P50    P90    P99    Max    Avg')
        print('------------------------------------------------------------------')
        print_pcts(nic, 'Xmit')
        print_pcts(gro, 'Net')
        print_pcts(softirq, 'SoftIRQ')
        print_pcts(free, 'Free')
        print_pcts(total, 'Total')

        if not options.verbose:
            return

        print()
        print('# Details of the selected packets, sorted by \'%s\':' %
                (options.sort))
        print(print_pkts(sort_pkts(pkts, options.sort), header=True), end='')
...
class AnalyzeNicbacklog:
    ...
    def output(self):
        ...
        # nic_pkts:    # packets owned by NIC at time
        # nic_bytes:   Bytes of data in packets owned by NIC at time
        # free_pkts:   # packets returned to Linux in the interval
        ...
        # node -> total bytes passed to NIC over trace
        node_total_in = defaultdict(lambda: 0)

        # node -> total bytes freed after return from NIC over trace
        node_total_freed = defaultdict(lambda: 0)
        ...
        print('AvgIn:    Average rate of data handed off to NIC (Gbps)')
        print('AvgFr:    Average rate of data freed after return from NIC (Gbps)')
        print()
        print('Node      AvgPkts  AvgKB MaxPkts  MaxKB MaxInP  MaxInD MaxFrP  MaxFrD   AvgIn   AvgFr')
        print('-------------------------------------------------------------------------------------')
        ...
        for pkt in itertools.chain(packets.values(), tcp_packets.values()):
            ...
            if 'free_tx_skb' in pkt:
                t2 = pkt['free_tx_skb']
                node_total_freed[node] += length
            else:
                t2 = traces[node]['last_time']
            delta_t = t2 - t1
            node_nic_pkts[node] += delta_t
            node_nic_bytes[node] += delta_t * length
        ...
        for node in get_sorted_nodes():
            ...
            # the posession of the NIC) in increasing order of free time.
            # Entries are <free, index, pkt> where free is the packet's
            # free_tx_skb time (for sorting), index is the packet's index
            # in the list of all packets (for resolving sorting ties), and
            # pkt is the packet.
            active = []
            ...
            while cur < len(pkts) or len(active) > 0:
                ...
                while cur < len(pkts) and pkts[cur]['nic'] <= interval_end:
                    ...
                    cur += 1
                    in_pkts += 1
                    in_bytes += pkt['tso_length'] + get_hdr_length(pkt)
                ...
                while len(active) > 0 and active[0][0] < interval_end:
                    ...
                    free_pkts += 1
                    free_bytes += pkt['tso_length'] + get_hdr_length(pkt)
                ...
                interval_ix += 1
                if nic_bytes >= 200000:
                    node_intervals[node].append([interval_end, nic_pkts,
                            nic_bytes, free_pkts, free_bytes])
            ...
            print('%-10s %6d %6d %7d %6d %6d %7.2f %6d %7.2f %7.2f %7.2f' % (node,
                    ...
                    node_total_in[node]*8e-3/node_time,
                    node_total_freed[node]*8e-3/node_time))

        # Output a table showing stats for the intervals with the highest
        # and lowest free_bytes.
        print()
        print('Average interval statistics for each node, measured over %d '
                'usec intervals.' % (base_interval))
        print('For each node, intervals with at least 200 Kbytes of NIC data '
                'are selected;')
        print('from this group the 10% slowest intervals (those with fewest '
                'bytes freed)')
        print('and 10% fastest intervals (those with the most NIC bytes freed)'
                ' are selected,')
        print('and the following statistics are printed from each group:')
        print('Node:     Name of node')
        print('NicPS:    Average # packets owned by NIC at the end of '
                'slow intervals')
        print('NicKbS:   Average KB of data owned by NIC at the end of '
                'slow intervals')
        print('FreePS:   Average # packets freed during slow intervals')
        print('FreeKbs:  Average KB of data freed during slow intervals')
        print('NicPS:    Average # packets owned by NIC at the end of '
                'fast intervals')
        print('NicKbS:   Average KB of data owned by NIC at the end of '
                'fast intervals')
        print('FreePS:   Average # packets freed during fast intervals')
        print('FreeKbs:  Average KB of data freed during fast intervals')
        print()
        print('Node       NicPS NicKbS FreePS  FreeKbS    NicPF NicKbF FreePF  FreeKbF')
        print('-----------------------------------------------------------------------')
        for node in get_sorted_nodes():
            intervals = sorted(node_intervals[node], key=lambda t: t[4])
            slow = intervals[0:(len(intervals)//10)]
            fast = intervals[len(9*intervals)//10:]
            if len(intervals) < 10:
                print('%s has only %d intervals' % (node, len(intervals)))
                continue
            print('%-10s %5.1f %6.1f  %5.1f   %6.1f    %5.1f %6.1f  %5.1f   %6.1f' %
                    (node,
                    sum(t[1] for t in slow)/len(slow),
                    sum(t[2] for t in slow)/(1000*len(slow)),
                    sum(t[3] for t in slow)/len(slow),
                    sum(t[4] for t in slow)/(1000*len(slow)),
                    sum(t[1] for t in fast)/len(fast),
                    sum(t[2] for t in fast)/(1000*len(fast)),
                    sum(t[3] for t in fast)/len(fast),
                    sum(t[4] for t in fast)/(1000*len(fast)),))

#------------------------------------------------
# Analyzer: nicbacklog2
#------------------------------------------------
class AnalyzeNicbacklog2:
    """
    Prints a time-series analysis of backlog in the NIC (packets that
    have been passed to the NIC but not yet returned after transmission).
    This differs from the nicbacklog analyzer in that it analyzes
    the distribution of traffic between device queues. Requires the
    --data option.
    """
    ...
    def output(self):
        ...
        # Each iteration in this loops generates data for one node.
        for node in get_sorted_nodes():
            f = open('%s/nicbacklog2_%s.dat' % (options.data, node), 'w')
            f.write('# Node: %s\n' % (node))
            f.write('# Generated at %s.\n\n' %
                    (time.strftime('%I:%M %p on %m/%d/%Y')))
            f.write('# Statistics on NIC backlog (packets handed off to the '
            ...
            f.write('\n')
            f.write('Time     Pkts    KB TcpKB PktQs FreeQs   '
                    'Qid1 Pkts1   KB1 TcpKB1   '
                    'Qid2 Pkts2   KB2 TcpKB2   '
                    'Qid3 Pkts3   KB3 TcpKB3\n')
            ...
            # queue -> 1 for all queues for which a packet was freed (i.e.
            # transmitted) in the current interval.
            queues_freed = {}

            # queue -> count of bytes currently owned by this queue.
            queue_bytes = defaultdict(lambda: 0)

            # queue -> same as queue_bytes except count only TCP bytes.
            queue_tcp_bytes = defaultdict(lambda: 0)

            # queue -> count of packets currently owned by this queue.
            queue_packets = defaultdict(lambda: 0)
            ...
            while True:
                ...
                while t >= interval_end:
                    # Format info for the queues with the most data.
                    details = ''
                    printed = 0
                    nonzero = 0
                    for qid, bytes in sorted(queue_bytes.items(),
                            key = lambda t: t[1], reverse = True):
                        if bytes == 0:
                            break
                        nonzero += 1
                        if printed < 3:
                            details += '   %4d %5d %5d %6d' %  (qid,
                                    queue_packets[qid], queue_bytes[qid]//1000,
                                    queue_tcp_bytes[qid]//1000)
                            printed += 1

                    # Generate output line
                    f.write('%7.1f %5d %5d %5d %5d %6d%s\n' % (
                            interval_end, sum(queue_packets.values()),
                            sum(queue_bytes.values())//1000,
                            sum(queue_tcp_bytes.values())//1000,
                            nonzero, len(queues_freed), details))
                ...
                if free:
                    ...
                    if queue_packets[qid] == 0:
                        if queue_bytes[qid] != 0:
                            raise Exception('%9.3f: queue_bytes[%d] is %d but '
                                    'queue_packets is 0' % (t, qid,
                                    queue_bytes[qid]))
                else:
                    queue_packets[qid] += 1
                    queue_bytes[qid] += pkt['tso_length']
                    ...
                    heapq.heappush(active, [free, cur, pkt])

            f.close()

        print('\n---------------------')
        print('Analyzer: nicbacklog2')
        print('---------------------')
        print('See data files %s/nicbacklog2_*.dat' % (options.data))

#------------------------------------------------
# Analyzer: nicpkts
#------------------------------------------------
class AnalyzeNicpkts:
    """
    Generate a history for each node of the packets owned by the NIC
    (packets passed to the NIC but not yet returned after transmission),
    showing the state of the NIC queues at each point in time and the
    order in which packets are returned to Linux after transmission.
    ...
    def print_active(self, f, active, free_index):
        ...
        active:        List of packets currently owned by NIC
        free_index:    Index in active of the next packet to be freed;
                       this packet will be highlighted in the printout
        """
        ...
        for pkt in active:
            if num_this_line == 4:
                ...
                f.write(' ')
            if index == free_index:
                f.write('%19s' % ('<Freed packet>'))
            else:
                f.write('%9.3f:%3d:%5d' % (pkt['nic'], pkt['tx_qid'],
                        pkt['tso_length']))
            num_this_line += 1
            index += 1
        if num_this_line > 0:
    ...
    def output(self):
        ...
        print('\n--------------------')
        print('Analyzer: nicpkts')
        print('--------------------')
        print('See data files %s/nicpkts*.dat' % (options.data))
        ...
        # Each iteration in this loops generates data for one node.
        for node in get_sorted_nodes():
            f = open('%s/nicpkts_%s.dat' % (options.data, node), 'w')
            f.write('# Node: %s\n' % (node))
            f.write('# Generated at %s.\n\n' %
                    (time.strftime('%I:%M %p on %m/%d/%Y')))
            f.write('# Each block of lines shows the packets owned by the '
                    'NIC when the next\n')
            f.write('# packet was returned to Linux after transmission. The '
                    'first line contains\n')
            f.write('# the time when the next packet was freed, plus '
                    'information about the freed\n')
            f.write('# packet.\n')
            f.write('#\n')
            f.write('# The following lines describe each of the packets '
                    'owned by the NIC as\n')
            f.write('# a tuple <t:qid:length>:\n')
            ...
            f.write('# The freed packet is not displayed in the list; '
                    '\'<Freed packet>\' is displayed\n')
            f.write('# in its place to highlight its position in the list.\n')

            # Contains all packets currently owned by the NIC, in increasing
            # order of nic time.
            active = []
            ...
            # Each iteration of this loop handles the next 'free_tx_skb'
            # event, which includes adding packets to the active list,
            # printing the list, and removing the next packet to be freed.
            while cur < len(pkts):
                # Find the first free time among active packets
                min_free = 1e20
                min_index = -1
                for i in range(len(active)):
                    pkt = active[i]
                    if pkt['free_tx_skb'] < min_free:
                        min_index = i
                        min_free = pkt['free_tx_skb']

                # Add more packets to the active list until the free time
                # is reached; this could cause the first free time to change.
                while cur < len(pkts):
                    pkt = pkts[cur]
                    if pkt['nic'] >= min_free:
                        break
                    ...
                    active.append(pkt)
                    if pkt['free_tx_skb'] < min_free:
                        min_index = len(active) - 1
                        min_free = pkt['free_tx_skb']

                # Print information about the active list and the packet
                # that was just freed.
                pkt = active[min_index]
                f.write('\n%9.3f: qid %d, slot %d, %d bytes, queued at %.3f\n' %
                        (pkt['free_tx_skb'], pkt['tx_qid'], min_index,
                        pkt['tso_length'], pkt['nic']))
                self.print_active(f, active, min_index)
...
class AnalyzeNictx:
    ...
    def output(self):
        ...
        # Generate CDF of throughput for intervals.
        tput = [(i[3] * 8e-3 / options.interval) for i in intervals]
        tput.sort()
        ...
        # Generate CDF of active queues for intervals.
        ...
        # Generate CDF of input to the NIC for intervals.
        input = [(i[4] * 8e-3 / options.interval) for i in intervals]
        input.sort()
        y = [i / len(input) for i in range(len(input))]
        fig = plt.figure(figsize=[6,4])
        ax = fig.add_subplot(111)
        ax.set_xlim(0, 120)
        ...
        print('---------------')
        print('Analyzed %d Homa data packets, %d Homa grants, %d '
                'TCP packets.' % (type_counts['data'], type_counts['grant'],
                type_counts['tcp']))
        print('Analyzed %d intervals of length %d usecs (use --interval '
                'option to' % (len(intervals), options.interval))
        print('change interval length).')
        print('See files %s/nictx_*.pdf for plots.' % (options.plot))
...
class AnalyzePackets:
    ...
    def tt_nic_data(self, trace, t, core, peer, id, offset, tx_queue):
        ...
        p = packets[pkt_id(id, offset)]
        if not 'nic' in p:
            rpcs[id]['nic_data_pkts'].append(p)
        p['tx_node'] = trace['node']
        if not p['retransmits']:
            p['nic'] = t
            p['tx_queue'] = tx_queue
            ...
            for pkt in pkts:
                if not 'nic' in pkt or not 'gro' in pkt:
                    if pkt_type == 'tcp':
                        ...
                        if 'gro' in pkt:
                            t = pkt['gro']
                            if pkt['saddr'] in ip_to_node:
                                trace = traces[ip_to_node[pkt['saddr']]]
                        else:
                            t = pkt['nic']
                            if pkt['daddr'] in ip_to_node:
                                trace = traces[ip_to_node[pkt['daddr']]]
...
class AnalyzeRpcs:
    ...
    def analyze(self):
        ...
        for id, rpc in rpcs.items():
            ...
            if 'peer' in rpc:
                peer = rpc['peer']
                if (not peer in ip_to_node) and peer_rpc:
                    ip_to_node[peer] = peer_rpc['node']
...
class AnalyzeRxpkts:
    ...
    data packet received by that node, in time order. Requires the --data
    option. Packets will normally be sorted by the 'Xmit' column, but the
    --sort option can be used to specify a different column (or columns) to
    use for sorting.
    """
    ...
    def output(self):
        ...
        # Bucket all of the packets by receiving node.
        for pkt in itertools.chain(packets.values(), tcp_packets.values()):
            if not 'gro' in pkt:
                continue
            node_pkts[pkt['rx_node']].append(pkt)
        ...
        print('\n----------------')
        print('Analyzer: rxpkts')
        print('----------------')
        print('See data files rxpkts_*.dat in %s\n' % (options.data))
        ...
        for node in get_sorted_nodes():
            pkts = node_pkts[node]
            sort_pkts(pkts, sort_key)

            f = open('%s/rxpkts_%s.dat' % (options.data, node), 'w')
            f.write('# Node: %s\n' % (node))
            f.write('# Generated at %s.\n' %
                    (time.strftime('%I:%M %p on %m/%d/%Y')))
            f.write('# Data packets received by %s:\n' % (node))
...
class AnalyzeRxsnapshot:
    ...
    def collect_live_rpcs(node, t, receive):
        ...
        post_xmit:         Lowest offset contained in a data packet with
                           'xmit' >= target time
        pre_nic and post_nic:
                           Same, except measured with 'nic' instead of 'xmit'
        ...
        for id, live_rpc in live_rpcs.items():
            ...
            for type in ['copied', 'softirq', 'gro', 'nic', 'xmit']:
                pre_field = 'pre_' + type
            ...
            if (unsched > 0 and live_rpc['pre_xmit'] > unsched and
                    live_rpc['pre_xmit'] > live_rpc['pre_grant_softirq']):
                # We sent unscheduled packets: they must have been granted.
                live_rpc['pre_grant_softirq'] = live_rpc['pre_xmit']
            ...
            if live_rpc['pre_grant_softirq']:
                ...
                live_rpc['sort_grant_softirq'] = live_rpc['pre_xmit']
            ...
            for pkt in live_rpc['pkts']:
                if (('nic' in pkt) and (not 'gro' in pkt)
                        and pkt['rx_node']
                        and (pkt['nic'] >= traces[pkt['rx_node']]['first_time'])
                        and ((options.time - pkt['nic']) > 200)
                        and (options.time < traces[pkt['rx_node']]['last_time'])):
                    live_rpc['lost'] += 1
    ...
    def get_sorted_ids(live_rpcs):
        ...
        sorted_ids = sorted(sorted_ids,
                key = lambda id : sort_key(live_rpcs, id, 'pre_nic'))
        sorted_ids = sorted(sorted_ids,
    ...
    def output(self):
        ...
        print('Xmit:      Offset just after last byte that has been '
                'passed to ip*xmit')
        print('Nic:       Offset just after last byte that has been queued '
                'in the NIC')
        ...
        # Bytes that have been passed to ip*xmit but not yet processed
        # by SoftIRQ
        total_live = 0
        ...
        for id, live_rpc in live_rpcs.items():
            ...
            # Envelope of all offsets in transit at the reference time (passed
            # to ip*xmit but not processed by SoftIRQ).
            min_start = None
            max_end = 0
            ...
            if live_rpc['pre_xmit'] < granted_at_t:
                max_end = live_rpc['pre_xmit']
            ...
            # There may also be unscheduled bytes for which we have no
            # packet records.
            if granted_at_t == 0 and live_rpc['pre_xmit'] > max_end:
                num_bytes = live_rpc['pre_xmit'] - max_end
                unsched += num_bytes
...
class AnalyzeSync:
    """
    Rewrite the timetrace files so that their clocks are synchronized.
    To do this, analyze the timetstamps for packet sends and receives and
    use this information to compute clock offsets between the nodes. If
    --no-update is specified then clock offset information is printed but
    ...
    def __init__(self, dispatcher):
        ...
        # the packet was a grant. If a packet is sent multiple times, time is
        # the earliest send time (avoids confusion caused by retransmissions)
        self.tx_pkts = {}

        # rpc_id:offset[g] -> <time, node> for each Homa data packet or grant
        # that was received. rpc_id is the id on the sender and node is the
        # node that received the packet. A 'g' suffix after the offset indicates
        # that the packet was a grant. If a packet is received multiple times,
        ...
        # busy packets were received for rpc_id (rpc_id is the id on the
        # receiver).
        self.ctl_rx = defaultdict(list)

        # pkt_id -> <time, node> for each TCP packet sent. pkt_id is a unique
        # identifier for the packet: source:dest:sequence:bytes. If the same id
        # is transmitted multiple times, the first time is used.
        self.tcp_tx = {}

        # pkt_id -> <time, node> for each TCP packet received. pkt_id has the
        # same structure as for tcp_tx. If the same id is received multiple
        # times, the last time is used.
        self.tcp_rx = {}

        # List of <time, snode, raddr> with one entry for each FREEZE packet
        # sent. Time is the unadjusted time on the sender when the packet was
        # sent. snode is the sending node, raddr is the receiver's IP address.
        self.freeze_tx = []

        # node -> <time, saddr>. time is the unadjusted time on node when
        # when the last freeze packet was received by it. saddr is the sender
        # IP address.
        self.freeze_rx = {}

        # Node -> dictionary with packet counts for that node:
        # homa_tx:    Homa data packets sent
        # homa_rx:    Homa data packets received
        # grant_tx:   Homa grant packets sent
        # grant_rx:   Homa grant packets received
        # ctl_tx:     Homa busy/resend packets sent
        # ctl_rx:     Homa busy/resend packets received
        # freeze_tx:  Homa freeze packets sent
        # freeze_rx:  Homa freeze packets received
        # tcp_tx:     TCP packets sent
        # tcp_rx:     TCP packets received
        self.node_pkts = defaultdict(lambda: defaultdict(lambda: 0))

        # rpc_id -> IP address of the node that processed the RPC with that id.
        # Needed to compute addr_node.
        self.id_addr = {}

        # rpc_id -> node that processed the RPC with that id. Used to compute
        # addr_node.
        self.id_node = {}

        # IP address -> node name; computed from id_addr and id_node.
        self.addr_node = {}
    ...
    def tt_gro_ctl(self, trace, t, core, peer, id, type):
        ...
        if type == '0x16':
            ...
            self.node_pkts[node]['freeze_rx'] += 1
        elif type == '0x12' or type == '0x14':
            self.ctl_rx[id].append(t)
            self.node_pkts[node]['ctl_rx'] += 1
        self.id_node[id] = node
        self.id_addr[id ^ 1] = peer
    ...
    def analyze(self):
        ...
        # Compute the mapping from address to node.
        for id, addr in self.id_addr.items():
            if id in self.id_node:
                self.addr_node[addr] = self.id_node[id]

        # Compute the mapping from node name to id.
        nodes = get_sorted_nodes()
        for i in range(len(nodes)):
    ...
    def find_delays_alt(self, delays):
        ...
        time to test it, so it is probably buggy.
        """
        ...
        for send_time, fsend_node, raddr in self.freeze_tx:
            # Compute freeze delay.
            if not raddr in self.addr_node:
                continue
            frecv_node = self.addr_node[raddr]
            if not frecv_node in self.freeze_rx:
                continue
            recv_time, saddr = self.freeze_rx[frecv_node]
            if (not saddr in self.addr_node or
                    not self.addr_node[saddr] == fsend_node):
                continue
    ...
    def get_offsets(self, delays):
        ...
        no minimum offset could be computed.

        max_offsets: the maximum valid clock offset for each node, or None if
        no maximum offset could be computed. If max_offset[i] < min_offset[i]
        it means there were errors in the input data.

        stats: a list containing one element for each iteration of the
        computation, indicating how many times a min_offset or max_offset
        ...
        while True:
            ...
            for i in range(len(min_offsets)):
                for j in range(1, len(min_offsets)):
    ...
    def output(self):
        ...
        print('Span:      Elapsed time between first and last timetrace records (ms)')
        print('HomaTx:    Total number of Homa data packets sent')
        print('HomaRx:    Total number of Homa data packets received (there '
                'will be more')
        print('           packets received than sent due to TSO on senders)')
        print('GrTx:      Total number of Homa grant packets sent')
        print('GrRx:      Total number of Homa grant packets received')
        print('CtlTx:     Total number of Homa busy/resend packets sent')
        print('CtlRx:     Total number of Homa busy/resend packets received')
        print('FrTx:      Total number of Homa freeze packets sent')
        ...
        for node in nodes:
            ...
            print('%-12s %7.1f %6d %6d %5d %5d %5d %5d %4d %4d %6d %6d' % (
                    ...
                    self.node_pkts[node]['homa_rx'],
                    self.node_pkts[node]['grant_tx'],
                    self.node_pkts[node]['grant_rx'],
                    self.node_pkts[node]['ctl_tx'],
                    self.node_pkts[node]['ctl_rx'],
                    self.node_pkts[node]['freeze_tx'],
                    self.node_pkts[node]['freeze_rx'],
        ...
        for i in range(len(stats)):
            print('%3d   %6d' % (i, stats[i]))

        print('\nClock offsets for each node:')
        print('Node:      Name of node')
        print('Min:       Minimum allowable Offset for Node (usecs)')
        print('Max:       Maximum allowable Offset for Node (usecs)')
        print('Range:     Max - Min (negative means error in data)')
        print('Offset:    Add this to clock values for Node')
        print('\nNode            Min      Max    Range   Offset')
        bad_nodes = []
        offsets = []
        for i in range(len(nodes)):
            min_str = 'N/A'
            ...
            offset = None
            if min_offsets[i] != None:
                min_str = '%.1f' % min_offsets[i]
            if max_offsets[i] != None:
                max_str = '%.1f' % max_offsets[i]
            if min_offsets[i] != None and max_offsets[i] != None:
                rng = max_offsets[i] - min_offsets[i]
                rng_str = '%.1f' % rng
                if rng >= 0:
                    offset = (min_offsets[i] + max_offsets[i]) / 2
                    offset_str = '%.1f' % offset
            print('%-10s %8s %8s %8s %8s' % (nodes[i], min_str, max_str,
                    rng_str, offset_str))
            offsets.append(offset)
            if offset == None:
                bad_nodes.append(nodes[i])

        if bad_nodes:
            raise Exception('Couldn\'t compute clock offsets for the '
                    'following nodes: %s' % (', '.join(bad_nodes)))
        ...
        if options.update:
            ...
            for i in range(1, len(nodes)):
                node = nodes[i]
                offset = offsets[i]
                file = traces[node]['file']
                print("Rewriting %s with offset %.1f usec" % (file, offset),
                        file=sys.stderr)
                src = open(file)
                ...
                for line in src:
                    ...
                    if not match:
                        ...
                        dst.write('%9.3f us %s\n' % (time, match.group(2)))
                dst.close()
                os.rename(dst.name, file)

#------------------------------------------------
# Analyzer: tcp_rpcs
#------------------------------------------------
class AnalyzeTcp_rpcs:
    """
    Print information about RPCs that were transmitted with TCP. The options
    --msglen, --rpc-start, and --rtt may be used to filter the RPCs to print.
    By default the RPCs are printed in order of start time, but that may be
    changed with the --sort option. The --sort option is a list of the
    column names Start, End, and Rtt; the RPCs will be sorted by each keyword
    in order before printing. If --verbose is specified then the packets from
    ...
    def __init__(self, dispatcher):
        ...
        self.rpcs = defaultdict(list)

        # "source dest" -> list of <time, sequence, length> tuples for
        # all of the recvmsg completions for this stream. Time is the time
        # when recvmsgc completed, sequence is the sequence number
        # just after the last one returned, and length is the number of
        # bytes returned.
    ...
    def tt_tcp_sendmsg(self, trace, t, core, source, dest, msg_length,
        ...
        # we don't have enough information to pair request and response
        # messages, so create separate "rpcs" for each, pretending all
        # messages are requests. The analyze method will combine requests
        # and responses into a single entry.
        rpc = {
            'client': source,
            'server': dest,
            'req_send': t,
            'req_length': msg_length,
            'req_seq': sequence,
            'req_end_seq': sequence + msg_length,
            'slot': slot,
            'req_pkts': [],
            'resp_pkts': []
        }
    ...
    def del_rpc(self, rpc):
        ...
        msg:     Entry to remove (it's incomplete: describes either a
                 request or a response but not both)
        """
        global tcp_rpcs
    ...
    def merge(self, request, response):
        ...
        request:      Entry in tcp_rpcs that describes a request message
        response:     Entry in tcp_repcs that describes the response
                      corresponding to request. This will be deleted.
        """
        request['resp_send'] = response['req_send']
        request['resp_seq'] = response['req_seq']
        request['resp_length'] = response['req_length']
        request['resp_end_seq'] = response['req_end_seq']
        request['resp_pkts'] = response['req_pkts']
        if 'req_recvd' in response:
    ...
    def analyze(self):
        ...
        """
        global tcp_packets, tcp_rpcs

        # <source, dest> -> list of data packets (nonzero length) from
        # source to dest.
        stream_pkts = defaultdict(list)

        # Bucket TCP packets in the same way as self.messages.
        for pkt in tcp_packets.values():
            if pkt['length'] == 0:
                continue
            key = '%s %s' % (pkt['source'], pkt['dest'])
            stream_pkts[key].append(pkt)

        # Assign TCP packets to messages. Each iteration through this
        # loop processes a source-dest pair, working through messages
        # and packets in sequence order. A single packet could contain
        # parts of multiple messages.
        for key, rpcs in self.rpcs.items():
            rpcs.sort(key = lambda rpc: rpc['req_seq'])
            pkts = sorted(stream_pkts[key],
                    key = lambda pkt: pkt['seq_ack'])
            pkt_ix = 0

            for rpc in rpcs:
                rpc_start = rpc['req_seq']
                rpc_end = rpc['req_end_seq']
                while pkt_ix < len(pkts):
                    pkt = pkts[pkt_ix]
                    pkt_start = pkt['seq_ack']
                    pkt_end = pkt_start + pkt['length']
                    if pkt_end <= rpc_start:
                        pkt_ix += 1
                        continue
                    if pkt_start >= rpc_end:
                        break
                    rpc['req_pkts'].append(pkt)
                    if pkt_end <= rpc_end:
                        pkt_ix += 1
                    else:
                        break

        # Add req_gro times to RPCs.
        for key, rpcs in self.rpcs.items():
            rpcs.sort(key = lambda rpc: rpc['req_end_seq'])
            recvs = sorted(self.recvs[key], key = lambda t: t[1],
                    reverse = True)

            for rpc in rpcs:
                while recvs:
                    t, sequence, length = recvs[-1]
                    rpc_end = rpc['req_end_seq']
                    if sequence < rpc_end:
                        recvs.pop()
                        continue
                    if sequence - length < rpc_end:
                        rpc['req_recvd'] = t
                    break
        ...
        for key, rpcs in list(self.rpcs.items()):
            ...
            slot_rpcs = defaultdict(list)

            for rpc in rpcs:
                if 'response' in rpc:
                    continue;
                slot_rpcs[rpc['slot']].append(rpc)
            source, dest = key.split()
            for rpc in self.rpcs[f'{dest} {source}']:
                if not 'response' in rpc:
                    continue
                slot_rpcs[rpc['slot']].append(rpc)
            for rpcs in slot_rpcs.values():
                rpcs.sort(key = lambda msg: msg['req_send'])
                request = None
                for rpc in rpcs:
                    if 'response' in rpc:
                        if (request == None or
                                rpc['req_send'] <= request['req_send']):
                            # Unmatchable response
                            self.del_rpc(rpc)
                            continue
                        self.merge(request, rpc)
                        request = None
                    else:
                        if request != None:
                            # Unmatchable request
...
class AnalyzeTemp:
    ...
    def output(self):
        ...
        events = []
        for rpc in rpcs.values():
            if not rpc['nic_data_pkts']:
                continue
            if not 'out_length' in rpc:
                continue
            pkt = rpc['nic_data_pkts'][0]
            if not 'tx_node' in pkt:
                continue
            tx_node = pkt['tx_node']
            offset = pkt['offset']
            start = pkt['nic'] if offset == 0 else traces[tx_node]['first_time']
            pkt = rpc['nic_data_pkts'][-1]
            if pkt['offset'] + pkt['tso_length'] >= rpc['out_length']:
                end = pkt['nic']
            else:
                end = traces[tx_node]['last_time']
            if start != end:
                node_events[tx_node].append([start, 'start', rpc])
                node_events[tx_node].append([end, 'end', rpc])

        for node in get_sorted_nodes():
            events = node_events[node]
            events.sort(key = lambda t: t[0])
            print('\n%s:' % (node))
            print('Time              Id       Peer  Event Active  Length')
            active = 0
            for t, type, rpc in events:
                if 'peer' in rpc and rpc['peer'] in ip_to_node:
                    peer = ip_to_node[rpc['peer']]
                else:
                    peer = ''
                length = rpc['out_length'] if 'out_length' in rpc else '??'
                if type == 'start':
                    active += 1
                elif type == 'end':
                    active -= 1
                ...
                print('%9.3f %10d %10s %6s   %4d %7s' % (
                        t, rpc['id'], peer, type, active, length))
...
class AnalyzeTemp2:
    ...
    def output(self):
        ...
        # Find all the packets that were sent from switch expt4 to expt5.
        # Also generate a list of interesting events for the selected
        # packets (nic, free, gro).
        expt4_nodes = {}
        for node in [1, 4, 8, 9, 10, 16, 17, 18, 24, 28, 32, 36, 43, 46, 47]:
            expt4_nodes['node%d' % node] = 1

        # Packets sent from expt4 to expt5.
        xpkts = []

        # List of [time, event, pkt], where event is one of 'nic', 'free', and
        # 'gro', time is the time when that event occurred, and pkt is the
        # full packet.
        events = []
        ...
        for pkt in packets.values():
            if not 'tx_node' in pkt or not 'rx_node' in pkt:
                continue
            if not 'nic' in pkt or not 'gro' in pkt or not 'free_tx_skb' in pkt:
                continue
            rx_node = pkt['rx_node']
            if not pkt['tx_node'] in expt4_nodes or rx_node in expt4_nodes:
                continue
            xpkts.append(pkt)
            events.append([pkt['nic'], 'nic', pkt])
        ...
        # Total bytes that have been passed to the NIC but not received.
        nic_backlog = 0

        # Total bytes that have been freed but not received.
        free_backlog = 0

        # Total bytes received in current interval.
        received = 0

        # Total bytes received on node 15 in the current interval.
        received15 = 0

        # Total bytes handed off to NICs in the current interval.
        sent = 0

        # free_backlog bytes that are destined for node15.
        backlog15 = 0

        # Largest delay from free to gro for any packet with gro in the
        # current interval.
        max_free_gro = 0

        # Node -> time of most recent arrival in GRO from a node on the
        # the expt4 switch.
        last_gro = defaultdict(lambda: None)

        # List of <t, node, gap> for long gaps between GRO arrivals on
        # a node. t is the time when GRO occurred, node is the node where
        # the gap occurred, gap is the time since the last GRO from an expt4
        # node.
        gro_gaps = []

        # Analyze the events and print information for each interval.
        events.sort(key = lambda t: t[0])
        ...
        for t, event, pkt in events:
            while t >= interval_end:
                ...
                # trace data for all nodes.
                if True or t > last_start:
                    # Print interval data.
                    print('%8.1f   %6.0f     %6.0f   %6.1f  %6.1f  %6.0f  %7.1f  %6.1f' % (
                            interval_end, nic_backlog * 1e-3,
                            free_backlog * 1e-3,
                            (sent * 8 / options.interval) * 1e-3,
                            (received * 8 / options.interval) * 1e-3,
                            backlog15 * 1e-3, max_free_gro,
                            (received15 * 8 / options.interval) * 1e-3,))
                sent = 0
                received = 0
                received15 = 0
                max_free_gro = 0
            ...
            if event == 'nic':
                ...
                if delay > max_free_gro and pkt['priority'] == 7:
                    max_free_gro = delay
                last = last_gro[rx_node]
                if last != None and t - last > 200:
                    gro_gaps.append([t, rx_node, t - last])
                last_gro[rx_node] = t

        print('\nLong GRO gaps that passed from expt4 to expt5:')
        print('Gap        Node              Time')
        for t, node, gap in sorted(gro_gaps, key=lambda t: t[2], reverse=True):
            if node in expt4_nodes:
                node += '*'
            print(' %6.1f   %-10s %9.3f' % (gap, node, t))

        # Node -> list of GRO times for packets that came from within the
        # node's switch.
        node_gros = defaultdict(list)
        for pkt in packets.values():
            if not 'gro' in pkt or not 'tx_node' in pkt or not 'rx_node' in pkt:
                continue
            tx_node = pkt['tx_node']
            rx_node = pkt['rx_node']
            if tx_node in expt4_nodes:
                if not rx_node in expt4_nodes:
                    continue
            else:
                if rx_node in expt4_nodes:
                    continue
            node_gros[rx_node].append(pkt['gro'])
        gro_gaps = []
        for node in get_sorted_nodes():
            gros = sorted(node_gros[node])
            for i in range(1, len(gros)):
                gap = gros[i] - gros[i-1]
                if gap > 50:
                    gro_gaps.append([gros[i], node, gap])
        print('\nLong GRO gaps within a switch:')
        print('Gap        Node              Time')
        for t, node, gap in sorted(gro_gaps, key=lambda t: t[2], reverse=True)[0:50]:
            if node in expt4_nodes:
                node = node + '*'
            print(' %6.1f   %-10s %9.3f' % (gap, node, t))
...
class AnalyzeTemp3:
    ...
    def print_gaps(self, events):
        ...
        # Node -> start time for that node: either the most recent GRO time
        # for the node, or the most recent time when the number of inbound
        # packets went from 0 to 1, whichever is latest.
        node_start = defaultdict(lambda: None)

        # Node -> number of packets inbound for the node (nic
        # has occurred but not gro).
        node_active = defaultdict(lambda: 0)

        # Node -> number of inbound packets at the most recent start time.
        node_start_active = {}

        # Node -> number of new packets passed to the NIC for the node since
        # node_start.
        node_xmits_in_gap = defaultdict(lambda: 0)

        # List of [t, node, gap, start_active, xmits_in_gap] for long GRO gaps:
        # t:              Time when GRO received a packet to end the gap.
        # node:           Node that received the packet.
        # gap:            Length of the gap (usecs).
        # start_active:   Number of inbound packets when gap started.
        # xmits_in_gap:   Number of new packets passed to nic during gap.
        gaps = []

        for t, event, pkt in sorted(events, key=lambda t: t[0]):
            node = pkt['rx_node']
            if event == 'nic':
                node_active[node] += 1
                if node_active[node] == 1:
                    node_start[node] = t
                    node_start_active[node] = 1
                    node_xmits_in_gap[node] = 0
                else:
                    node_xmits_in_gap[node] += 1
            elif event == 'gro':
                if node_start[node] != None:
                    gap = t - node_start[node]
                    if gap > 20:
                        gaps.append([t, node, gap, node_start_active[node],
                                node_xmits_in_gap[node]])
                node_active[node] -= 1
                if node_active[node] == 0:
                    node_start[node] == None
                else:
                    node_start[node] = t
                    node_start_active[node] = node_active[node]
                    node_xmits_in_gap[node] = 0

        # Print out the largest gaps.
        print('Gap          Time  Node      StartActive  GapXmits')
        gaps.sort(key=lambda t: t[2], reverse=True)
        for t, node, gap, start_active, gap_xmits in gaps[:50]:
    ...
    def output(self):
        ...
        # List of [time, event, pkt] for packets that don't only involve
        # one switch. Event is 'nic' or 'gro', time is the time when that
        # event occurred, and pkt is the full packet.
        events = []

        # Same as events except for packets that cross from one switch to
        # another.
        xevents = []
        ...
        for pkt in itertools.chain(packets.values(), tcp_packets.values()):
            if not 'tx_node' in pkt or not 'rx_node' in pkt:
                ...
                alive[0] += 1
            if not 'gro' in pkt or not 'nic' in pkt:
                continue
            else:
                alive[1] += 1
            rx_node = pkt['rx_node']
            selected_list = events
            if pkt['tx_node'] in expt4_nodes:
                if not pkt['rx_node'] in expt4_nodes:
                    selected_list = xevents
            elif pkt['rx_node'] in expt4_nodes:
                    selected_list = xevents
            selected_list.append([pkt['nic'], 'nic', pkt])
            selected_list.append([pkt['gro'], 'gro', pkt])
        print('alive: %s' % (alive))

        print('Largest gaps for packets within a switch:')
        self.print_gaps(events)
        print('\nLargest gaps for packets that cross between switches:')
        self.print_gaps(xevents)
...
class AnalyzeTxsnapshot:
    ...
    def get_sorted_ids(self, live_rpcs):
        ...
        sorted_ids = sorted(live_rpcs.keys(),
                key = lambda id : live_rpcs[id]['pre_xmit'],
                reverse = True)
        sorted_ids = sorted(sorted_ids,
    ...
    def output(self):
        ...
        print('Xmit:      Offset just after last data byte that has been '
                'passed to ip*xmit')
        print('Nic:       Offset just after last data byte that has been '
                'queued in the NIC')
        print('Gro:       Offset just after last data byte that has been '
                'processed by GRO')
        print('SoftIrq:   Offset just after last data byte that has been '
        ...
        print('GGro   GSoft    Xmit     Nic     Gro SoftIrq  Copied Incoming Lost')
        ...
        for id in sorted_ids:
            ...
            window = live_rpc['pre_grant_softirq'] - live_rpc['pre_xmit']
            if window > 0:
                window = str(window)
            else:
                window = ""
            print('%10d %-10s %7s %7s %7s %7s ' % (id, get_rpc_node(id^1),
                    ...
                    window,
                    tx_rpc['out_length'] - live_rpc['pre_xmit']
                    if tx_rpc['out_length'] != None else "",
                    str(live_rpc['pre_grant_xmit'])
                    if live_rpc['pre_grant_xmit'] > 0 else ""), end='')
            print('%7s %7s %7d %7d %7d %7d %7d  %7d %4d' % (
                    str(live_rpc['pre_grant_gro'])
...
parser.add_option('--filter', dest='filter', default=None,
        metavar='FUNC', help='\'filter_FUNC\' is the name of a function in the '
        'analyzer class; used by some analyzers as an additional filter for '
        'packets.')
parser.add_option('--gbps', dest='gbps', type=float, default=100.0,
        metavar='G', help='Link speed in Gbps (default: 100); used by some '
        'analyzers.')
parser.add_option('--grolat', dest='grolat', default=None,
        metavar='L', help='Used by some analyzers to filter packets based on '
        'the elapsed time from when the packet was passed to ip*xmit until '
        'it was received by GRO on the destination; it can contain either a '
        'single floating-point value (minimum latency) or two values (min and '
        'max, inclusive).')
parser.add_option('-h', '--help', dest='help', action='store_true',
                  help='Show this help message and exit')
parser.add_option('--interval', dest='interval', type=int, default=50,
...
parser.add_option('--min', dest='min', type=float, default=None,
        ...
        'specific meaning depends on analyzer')
parser.add_option('--msglen', dest='msglen', default=None,
        metavar='L', help='Used by some analyzers to filter packets based on '
        'message length; it can contain either a single integer value '
        '(largest allowable length) or two values (min and max, inclusive).')
...
parser.add_option('--no-update', action='store_false', default=True,
        dest='update',
        help='Used by some analyzers to control whether updates are made; '
        'if specified, updates will not be made')
...
parser.add_option('--rtt', dest='rtt', default=None,
        metavar='T', help='Used by some analyzers to filter RPCs based on '
        'end-to-end round trip time; contains two values (min and max, '
        'inclusive).')
...
parser.add_option('--same-gro-core', dest='same_gro_core', action="store_true",
        ...
        'same core')
parser.add_option('--segs', action='store_true', default=False, dest='segs',
        help='By default some analyzers will consider only the first segment '
        'of packets that are segmented by TSO segmentation; if this option '
        'is specified then they will consider all of the derived segments')
parser.add_option('--sort', dest='sort', default=None,
        metavar='S', help='Used by some analyzers to select a field to use '
        'for sorting packets; legal values and the default depend on the'
        'analyzer')
parser.add_option('--threshold', dest='threshold', type=int, default=50,
