	...
	, lag(0)
{
	...
	double lambda = 1e09*(net_gbps/8.0)/(avg_length*client_ports);
	interval_dist = std::exponential_distribution<double>(lambda);
	...
	log(NORMAL, "Average message length %.1f KB (expected %.1fKB), "
			...
			avg_length*1e-3, avg_length*1e-3, lambda*1e-3,
			avg_length*lambda*8e-9);
	kfreeze_count = 0;
}
...
void homa_client::measure_unloaded(int count)
{
	...
	std::vector<int> dist = length_dist.sizes();
	int server = 0;
	int slot;
