...
# Copy out time that also includes time to delete skbuffs.
copy_out_time_with_del = 0
...
for line in f:
  ...
  match = re.match(' *([-0-9.]+) us \(\+ *([-0-9.]+) us\) \[C([0-9]+)\] '
      'finished freeing [0-9]+ skbs', line)
  if match:
    time = float(match.group(1))
    core = int(match.group(3))
    if core in last_copy_out_start:
        copy_out_time_with_del += time - last_copy_out_start[core]
