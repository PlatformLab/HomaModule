...
TEST_F(homa_incoming, homa_message_in_init__basics)
{
	...
	EXPECT_EQ(127, crpc->msgin.length);
...
TEST_F(homa_incoming, homa_message_in_init__message_too_long)
{
	...
	self->data.msg_length = htonl(HOMA_MAX_MESSAGE_LENGTH+1);
...
TEST_F(homa_incoming, homa_message_in_init__negative_message_length)
{
	...
	self->data.msg_length = htonl(-1);
	srpc = homa_rpc_alloc_server(&self->hsk, self->client_ip, &self->data.common);
	ASSERT_TRUE(IS_ERR(srpc));
	EXPECT_EQ(EINVAL, -PTR_ERR(srpc));
}
...
#ifndef __STRIP__ /* See strip.py */
TEST_F(homa_incoming, homa_request_retrans__send_start_msg)
{
	struct homa_rpc *crpc;

	self->homa.unsched_bytes = 5000;
	crpc = unit_client_rpc(&self->hsk, UNIT_OUTGOING, self->client_ip,
			       self->server_ip, self->client_port,
			       self->client_id, 10000, 100);
	ASSERT_NE(NULL, crpc);
	EXPECT_EQ(0, crpc->msgout.granted);
...
#ifndef __STRIP__ /* See strip.py */
	srpc->msgin.granted = srpc->msgin.recv_end;
...
#ifndef __STRIP__ /* See strip.py */
	EXPECT_STREQ("xmit RESEND 1000, 1000 @7; "
		     "xmit RESEND 4000, 2000 @7; "
		     "xmit RESEND 7000, 1000 @7",
			unit_log_get());
#else /* See strip.py */
	EXPECT_STREQ("xmit RESEND 1000, 1000; "
			"xmit RESEND 4000, 2000; "
			"xmit RESEND 7000, 1000; "
			"xmit RESEND 1400, 8600",
			unit_log_get());
#endif /* See strip.py */
}
...
TEST_F(homa_incoming, homa_request_retrans__no_granted_but_not_received_data)
{
	...
	EXPECT_EQ(1400, srpc->msgin.recv_end);
	unit_log_clear();
...
#endif /* See strip.py */
TEST_F(homa_incoming, homa_request_retrans__no_data_received_yet)
{
	struct homa_rpc *crpc = unit_client_rpc(&self->hsk,
			UNIT_OUTGOING, self->client_ip, self->server_ip,
			self->server_port, 99, 1000, 10000);
...
#else /* See strip.py */
	EXPECT_STREQ("xmit RESEND 0, -1", unit_log_get());
#endif /* See strip.py */
}
...
TEST_F(homa_incoming, homa_add_packet__packet_beyond_message_end)
{
	struct homa_rpc *crpc = unit_client_rpc(&self->hsk,
			UNIT_OUTGOING, self->client_ip, self->server_ip,
			self->server_port, 99, 1000, 1000);
...
TEST_F(homa_incoming, homa_add_packet__packet_before_gap)
{
	...
	self->data.seg.offset = htonl(0);
	skb = mock_skb_alloc(self->client_ip, self->server_ip, &self->data.common, 1400, 0);
	homa_add_packet(crpc, skb);
	kfree_skb(skb);
	EXPECT_EQ(2, skb_queue_len(&crpc->msgin.packets));
}
TEST_F(homa_incoming, homa_add_packet__packet_straddles_start_of_gap)
{
	...
	skb = mock_skb_alloc(self->client_ip, self->server_ip, &self->data.common, 1400, 1000);
	homa_add_packet(crpc, skb);
	kfree_skb(skb);
	EXPECT_EQ(2, skb_queue_len(&crpc->msgin.packets));
}
...
TEST_F(homa_incoming, homa_copy_to_user__basics)
{
	...
	ASSERT_NE(NULL, crpc);
	self->data.msg_length = htonl(4000);
	self->data.seg.offset = htonl(1400);
	homa_data_pkt(mock_skb_alloc(self->server_ip, self->client_ip,
				     &self->data.common, 1400, 101000), crpc);
	self->data.seg.offset = htonl(2800);
	homa_data_pkt(mock_skb_alloc(self->server_ip, self->client_ip,
				     &self->data.common, 1200, 201800), crpc);
...
TEST_F(homa_incoming, homa_copy_to_user__many_chunks_for_one_skb)
{
	...
	self->data.msg_length = htonl(4000);
	homa_data_pkt(mock_skb_alloc(self->server_ip, self->client_ip,
				     &self->data.common, 3000, 101000), crpc);
...
#ifndef __STRIP__ /* See strip.py */
TEST_F(homa_incoming, homa_dispatch_pkts__start_msg_creates_new_server_rpc)
{
	struct homa_start_msg_hdr h;
	struct homa_rpc *srpc;

	h.common.sport = htons(self->client_port);
	h.common.dport = htons(self->server_port);
	...
	EXPECT_EQ(1, unit_list_length(&self->hsk2.active_rpcs));
	srpc = list_first_entry(&self->hsk2.active_rpcs, struct homa_rpc,
				active_links);
	EXPECT_EQ(1000, srpc->msgin.length);
}
#endif
...
TEST_F(homa_incoming, homa_data_pkt__wrong_client_rpc_state)
{
	...
	self->data.seg.offset = htonl(1400);
	homa_data_pkt(mock_skb_alloc(self->server_ip, self->client_ip,
				     &self->data.common, 600, 1400), crpc);
...
TEST_F(homa_incoming, homa_data_pkt__send_cutoffs)
{
	...
	/* Try again, but this time no cutoffs should be sent because
...
TEST_F(homa_incoming, homa_data_pkt__handle_ack)
{
	...
	unit_log_clear();
	homa_rpc_lock(srpc2);
	homa_data_pkt(mock_skb_alloc(self->client_ip, self->server_ip, &self->data.common,
			1400, 0), srpc2);
...
TEST_F(homa_incoming, homa_resend_pkt__negative_length_in_resend_send_start_msg)
{
	...
	self->homa.unsched_bytes = 5000;
	srpc = unit_server_rpc(&self->hsk2, UNIT_OUTGOING, self->client_ip,
			       self->server_ip, self->client_port,
			       self->server_id, 2000, 20000);
	ASSERT_NE(NULL, srpc);
...
TEST_F(homa_incoming, homa_resend_pkt__no_need_to_clip_range)
{
	...
	ASSERT_NE(NULL, crpc);
	unit_log_clear();
	IF_NO_STRIP(crpc->msgout.granted = 2800);
...
#endif /* See strip.py */
TEST_F(homa_incoming, homa_resend_pkt__requested_data_hasnt_been_sent_yet)
{
	struct homa_resend_hdr h = {{.sport = htons(self->server_port),
			.dport = htons(self->hsk.port),
			.sender_id = cpu_to_be64(self->server_id),
...
TEST_F(homa_incoming, homa_need_ack_pkt__rpc_response_not_fully_received)
{
	...
	EXPECT_EQ(1, homa_metrics_per_cpu()->packets_received[
			NEED_ACK - DATA]);
#else /* See strip.py */
	EXPECT_STREQ("xmit RESEND 1400, 1600", unit_log_get());
#endif /* See strip.py */
}
...
TEST_F(homa_incoming, homa_ack_pkt__reduce_oversize_count)
{
	...
	skb = mock_skb_alloc(self->client_ip, self->server_ip, &h.common,
					  sizeof(ack), 0);
...
TEST_F(homa_incoming, homa_wait_shared__reap_when_nonblocking)
{
	struct homa_rpc *crpc;
	struct homa_rpc *rpc;
