...
#else
#define TT_BUF_MASK (tt_buffer_size - 1)
#endif
extern int       tt_buffer_size;

/* Represents a sequence of events, typically consisting of all those
 * generated by one thread.  Has a fixed capacity, so slots are reused
 * on a circular basis.  This class is not thread-safe.
 */
struct tt_buffer {
	/**
	 * The low-order bits of this value hold the index within events
	 * of the slot to use for the next tt_record call. High-order bits
	 * may be garbage (the value is incremented without checking for
	 * wraparound).
	 */
	atomic_t next_index;
...
{
	return atomic_read(&buffer->next_index) & TT_BUF_MASK;
}
